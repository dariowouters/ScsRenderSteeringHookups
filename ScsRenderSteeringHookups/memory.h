...
private:
    static uint8_t string_to_uint8(const std::string& str)
    {
        ...
        if (uint32_t ret; iss >> std::hex >> ret)
        {
...
struct pattern
{
    static uint64_t scan(const std::string s, const uint64_t start, const uint64_t length, const int occurrence = 1)
    {
